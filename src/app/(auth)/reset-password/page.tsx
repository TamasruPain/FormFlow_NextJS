  ...
  if (!token) {
    return (
      <motion.div
        ...
        initial="hidden"
        animate="show"
        className="bg-zinc-900/30 border border-zinc-900 rounded-3xl p-8 shadow-2xl backdrop-blur-2xl space-y-6 relative overflow-hidden text-center"
      >
        <div className="absolute top-0 inset-x-0 h-px bg-gradient-to-r from-transparent via-rose-500/50 to-transparent" />
        
        <motion.div variants={itemVariants} className="flex justify-center">
          <div className="h-12 w-12 rounded-full bg-rose-500/10 flex items-center justify-center border border-rose-500/20 text-rose-400">
        ...
        <motion.div variants={itemVariants} className="pt-2">
          <Button
            asChild
            className="w-full bg-zinc-900 border border-zinc-800 hover:bg-zinc-800 text-zinc-200 rounded-xl h-11 transition-all duration-300 font-semibold"
          >
    ...
    >
      ...
      {success && (
        ...
        >
          ...
          <p className="leading-relaxed opacity-90">Your credentials have been updated. Redirecting to the sign in page...</p>
        </motion.div>
      )}
          ...
          >
            ...
            <div className="space-y-3 bg-zinc-950/60 rounded-xl p-4 border border-zinc-900/80">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h4 className="text-xs font-bold text-zinc-200">Reset Token Authenticated</h4>
                  <p className="text-[9px] text-zinc-500">Authorized signature verified</p>
                </div>
              </div>
              ...
              <div className="pt-2 border-t border-zinc-900/60 flex items-center justify-between text-[10px] text-zinc-400">
                ...
                <span className="text-zinc-500">Secure Reset Port</span>
              </div>
            </div>

            {/* Password security tips */}
            <div className="space-y-2.5 pt-2 border-t border-zinc-800/40">
              <span className="text-[10px] font-bold text-zinc-500 uppercase tracking-wider block">Security Guidelines</span>
              <div className="space-y-1.5 text-[11px] text-zinc-400">
                ...
                <div className="flex items-center gap-2">
                  <span className="h-1.5 w-1.5 rounded-full bg-blue-400" />
                  <span>Avoid using common words or names</span>
                </div>
                <div className="flex items-center gap-2">
                  <span className="h-1.5 w-1.5 rounded-full bg-blue-400" />
                  <span>Combine numbers, symbols, and capitals</span>
                </div>
              </div>
            </div>
          </motion.div>
        ...
        <div className="relative z-10 w-full max-w-md mx-auto">
          <Suspense fallback={
            <div className="flex flex-col items-center justify-center min-h-[300px] text-zinc-400 gap-2">
              ...
              <p className="text-sm">Loading security token...</p>
            </div>
          }>
            <ResetPasswordForm />
          </Suspense>
